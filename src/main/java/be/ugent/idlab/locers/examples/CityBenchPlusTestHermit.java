...
public class CityBenchPlusTestHermit {
	...
	Thread.sleep(10000);
		...
		try (Stream<String> stream = Files.lines(Paths.get(fileName))) {
			...
			stream.limit(1000).forEach(p -> System.out.println(parseEvent(reasoner,manager,mapper.map(p),ontology)));
	...
	public static Set<OWLAxiom> parseEvent(Reasoner reasoner, OWLOntologyManager manager, String triples, OWLOntology ont) {
		if(triples==null) {
		...
		try {
			...
			Set<OWLIndividual> inds = eventOnt.individualsInSignature().collect(Collectors.toSet());
			Set<OWLAxiom> allResults = new HashSet<OWLAxiom>();
			long time1 = System.currentTimeMillis();
			manager.addAxioms(ont, event);
			reasoner.flush();
			for(OWLIndividual ind:inds) {
				Set<OWLClass> results = reasoner.getTypes(ind.asOWLNamedIndividual(),false).getFlattened();
				for(OWLClass result:results) {
					allResults.add(fact.getOWLClassAssertionAxiom(result,ind));
				}
			}
